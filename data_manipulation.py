...
class data_init(object):
    ...
    def __init__(self, csv_data):
        ...
        self.chart_df.insert(3, 'Monthly Income', self.preprocess_monthly_income(csv_data['Monthly Income']))

        self.chart_df_scaled = self.processed_df.copy()
        self.chart_df_scaled.insert(3, 'Monthly Income 1:100', self.cleaned_income.div(100), True)
       

    def preprocess_monthly_income(self, csv_data, is_scaled: bool = False):
        "Logic for monthly income column preprocessing"

        self.cleaned_income = csv_data.str.strip("USD").str.replace(',', '').astype(float)

        return self.cleaned_income
        
        """
        if is_scaled:
            income_1to100_scale = cleaned_income.div(100)
            return income_1to100_scale
        else:
...
class chart_types(data_init):
    ...
    def __init__(self, csv_data):
        ...
        #self.chart_df = self.chart_df
        #self.chart_df_scaled = self.chart_df_scaled
    ...
    def choose_chart_params(self):
        ...
        if input("Use scaled dataset? y / n ") == ('y' or 'Y' or 'yes' or 'Yes'):
            data_source = self.chart_df_scaled
        else:
            data_source = self.chart_df
        self.format_continent_data(data_source)
        ...
        match chart_type:
            ...
            case 'l'|'line'|'Line':
                ...
                if input("Specify Y-axis? y / n ") == ('y' or 'Y' or 'yes' or 'Yes'):
    ...
    def set_axis_params(self, used_axis):
        ...
        if 'x' in used_axis:
            try:
                self.x_axis = self.available_columns[input(f"Choose X column {self.available_columns} ")]
            except:
                self.x_axis = list(self.available_columns.values())[0]
        
        if 'y' in used_axis:
            try:
                self.y_axis = self.available_columns[input(f"Choose Y column {self.available_columns} ")]
            except:
    ...
    def format_continent_data(self, data_source):
        ...
        eu_dataframe = data_source[data_source.Continent == "EU"]
        na_dataframe = data_source[data_source.Continent == "NA"]
        as_dataframe = data_source[data_source.Continent == "AS"]
        af_dataframe = data_source[data_source.Continent == "AF"]
        oc_dataframe = data_source[data_source.Continent == "OC"]
